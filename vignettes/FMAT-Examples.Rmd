...
The **FMAT (Fill-Mask Association Test)** is a generic, probability-based method that employs masked language models (e.g., BERT) to measure conceptual associations. In contrast with the use of *cosine similarity* in the WEAT (Word-Embedding Association Test; Caliskan et al., 2017), the FMAT typically calculates the *log probability* of specific words that are likely to be filled in the blank `[MASK]` in language models such as BERT. The final index used in the FMAT is the **log probability ratio (LPR)**, which should often have been contrasted between attributes and between targets.
...
$$
\text{Log Probability Ratio} = \text{LPR}(w)_{a \text{ vs } b} = \log \frac {P(w|context_a)} {P(w|context_b)} = \log P(w|context_a) - \log P(w|context_b)
$$
...
q1b = FMAT_query(
  c("[MASK] is a nurse.", "[MASK] works as a nurse."),
  MASK = .(Male=s("Albert, Bruce, Charles, Daniel, Smith"),
           Female=s("Alice, Betty, Chloe, Daisy"))
)
d1b = FMAT_run(models, q1b, progress="none")  # progress="text"
summary(d1b, mask.pair=FALSE) %>% kable()
## Linear Model
## contrasts: contr.sum
...
d2a = FMAT_run(models, q2a, progress="none")  # progress="text"
summary(d2a) %>% kable()
## Linear Model
## contrasts: contr.sum
...
d2b = FMAT_run(models, q2b, progress="none")  # progress="text"
summary(d2b) %>% kable()
## Mean Score for Each Item
d2b.sum = summary(d2b)
d2b.sum$T_word = as_factor(str_remove(d2b.sum$T_word, "a |an "))
d2b.sum[, .(MASK = MASK[1], LPR_mean = mean(LPR)), keyby=T_word] %>% kable()
```
...
d3a = FMAT_run(models, q3a, progress="none")  # progress="text"
summary(d3a) %>% kable()
## Linear Mixed Model
## (more conservative and suitable for large number of random items)
...
q3b = FMAT_query(
  ...
  MASK = .(H="high", L="low"),
  TARGET = .(Young=s("young, younger"),
             Old=s("old, older")),
  ATTRIB = .(IND=s("individualism, being individualists"),
             COL=s("collectivism, being collectivists"))
)
d3b = FMAT_run(models, q3b, progress="none")  # progress="text"
summary(d3b) %>% kable()
## Linear Model
## contrasts: contr.sum
...
## Ex.4: `[MASK]` = years (for detecting changes over time)
...
d4.sum[, LPR_relative := LPR - LPR[1], by=model]
